...
public class AllSentinelEntriesFilterSpecificationTests
{
    [Test]
    public void WhenSpecificationIsCreated_OrdersByIdDescending()
    {
        var spec = new AllSentinelEntriesFilterSpecification();

        var result = spec.Evaluate(GetTestCollection());

        result.Select(s => s.Id).Should().ContainInConsecutiveOrder(new List<int>{8,5,4,3,2,1});
    }
